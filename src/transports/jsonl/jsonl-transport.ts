...
export async function* runJsonlTransport<TItem>(
  ...
  async function* runOnce(runPlan: ProviderLaunchPlan): AsyncGenerator<AgentEvent> {
    ...
    void processHandle.waitForExit().then(({ code, signal, timedOut }) => {
      ...
      if (timedOut) {
        queue.push({
          ...
          message: `Process timed out after ${runPlan.timeoutMs}ms.`,
        });
  ...
  if (plan.fallbackPlan) {
    ...
    for await (const event of runOnce(plan)) {
      firstAttempt.push(event);
    }
    const failedBeforeCompletion = firstAttempt.some(
      (event) => event.type === "error" && event.code === "process_exit_nonzero",
    );
    if (failedBeforeCompletion) {
      yield* runOnce(plan.fallbackPlan);
      return;
    }
    yield* firstAttempt;
    return;
